...
import { Alfred } from "./Character/Alfred";
...
const MIN_DISTANCE_JACOB = 2;
const AlfredNpc = ({ name, position, heroRef }) => {
  ...
  useEffect(() => {
    actions[actionState.Alfred]?.reset()?.fadeIn(1)?.play();
    return () => {
      actions[actionState.Alfred]?.reset().fadeOut(1);
    };
  }, [actionState?.Alfred]);
  useEffect(() => {
    if (client?.isProximity) {
      updateClient(client);
      if (client?.isTalking) {
        updateActionState.Alfred("talking");
      } else {
        updateActionState.Alfred("listening");
      }
  ...
  useFrame((state, delta) => {
    const alfredWorldPosition = ref.current.getWorldPosition(
      new THREE.Vector3()
    );
    const heroWorldPosition = heroRef.current.getWorldPosition(
      new THREE.Vector3()
    );
    let distance = squaredDistance(alfredWorldPosition, heroWorldPosition);
    if (client?.isProximity) {
      ref.current.lookAt(heroWorldPosition);
      if (distance > MIN_DISTANCE_JACOB) {
        client.setIsProximity(false);
        updateActionState.Alfred("idle");
